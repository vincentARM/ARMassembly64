...
qColorType:            .skip 8
qBit_depth:            .skip 8
tablePointeurs:        .skip 8 * 2000   // à allouer en fonction des tailles
...
main:                                   // programme principal
    ...
    ldr x0,qAdrqAdrFicName            // nom du fichier image
    ldr x0,[x0]
    bl chargePNG
...
//.equ  PNG_LIBPNG_VER_STRING, "1.6.36"    // 
chargePNG:                    // fonction
    stp x20,lr,[sp,-16]!         // save  registres
    stp x21,x24,[sp,-16]!        // save  registres
    stp x26,x27,[sp,-16]!        // save  registres

    /* ouverture fichier en utilisant fonction C fopen */
    ldr x1,qAdrszModeOpen        // mode
    bl fopen
    cmp x0,0                     // si erreur
    ble erreur1
    mov x27,x0                   // save du File Descriptor
                                 // lecture, x0 contient le FD du fichier

    /* lecture de 8 octets pour vérifier la signature du fichier */
    ldr x0,qAdrsBuffer         // buffer de reception
    mov x1,1
    mov x2,8                   // nombre de caractère
    mov x3,x27                 // File descriptor
    bl fread                   // appel fonction de lecture du C
    cmp x0,0
    ble erreur2
    ldr x0,qAdrsBuffer         // adresse buffer
    affmemtit buffer x0 2
    mov x1,0
    ...
    mov x1,x27                  // FD du fichier ouvert
    mov x2,0
    bl png_init_io              // liaison entre les deux
    mov x0,x26                  // pointeur structure lecture
    mov x1,8                    // 8 bytes deja lu
    bl png_set_sig_bytes        // indication à la structure
    mov x0,x26                  // pointeur structure lecture
    ...
    mov x23,x0                 // récup type couleur
    affregtit colortype 0
    /* ici il faut prévoir éventuellement des modifications en fonction
...
1:
    ...
    ldr x0,qAdrtablePixels    // stockage adresse début pixel dans structure image
...
fermeture:
    mov x0,x27
    bl fclose                   // fermeture du fichier image png fonction C
    b 100f
erreur1:    
