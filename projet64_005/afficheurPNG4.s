...
/* amélioration programme précedent */
/* allocation place sur le tas */
/* chargement et affichage autre image */
...
/* structure de type  fichier image PNG   */
/* cette structure concatene une structure de type file 
   et une structure de type PNG   */
    .struct  0
imagePNG_name:              /* nom */
    .struct imagePNG_name + 256
imagePNG_datas:              /* donnees lues */
    .struct imagePNG_datas + 8
imagePNG_size:              /* taille */
    .struct imagePNG_size + 8
imagePNG_offset:              /* offset */
    .struct imagePNG_offset + 8
imagePNG_largeur:               /* largeur en pixel de l'image */
    .struct imagePNG_largeur + 8
imagePNG_hauteur:               /* hauteur en pixel de l'image */
    .struct imagePNG_hauteur + 8
imagePNG_debut_pixel:               /* pointeur début des pixels de l'image */
    .struct imagePNG_debut_pixel + 8
imagePNG_imageX11:
    .struct imagePNG_imageX11 + 8
imagePNG_fin:
/*******************************************/
...
szWhite:                 .asciz "white"
/* fichier image */
szNomImage1:             .asciz "dragon_form.png"
...
//.align 4
stIMGPNG:               .skip imagePNG_fin
.align 4
stIMGPNG1:               .skip imagePNG_fin
.align 4
...
main:                                   // programme principal // fonction
    ...
    ldr x1,qAdrstIMGPNG                 // structure fichier image PNG
    bl lectfichierPNG                   // lecture fichier image
    cbz x0,erreurGénérale
    ldr x0,qAdrstIMGPNG                 // structure fichier image PNG
    ...
    cbz x0,erreurGénérale
    ldr x1,qAdridFenSec2
    str x0,[x1]                        // save ident fenetre secondaire

    /* affichage de l'image dans la fenêtre secondaire */
                                        // xo contient l'adresse de la fenetre secondaire
    ldr x1,qAdrstIMGPNG                 // structure fichier image PNG
    mov x2,0
    mov x3,0
    bl affichageImagePNG

    /* chargement et affichage d'une autre image */
    ldr x0,qAdrszNomImage1             // nom du fichier image à charger
    ldr x1,qAdrstIMGPNG1               // structure fichier image PNG
    ldr x2,qAdridFenSec2
    ldr x2,[x2]                        // ident fenetre secondaire
    mov x3,50                          // position X
    mov x4,100                         // position Y
    bl chargementImage1
    /* affichage meme image */
    ldr x0,qAdridFenSec2
    ldr x0,[x0]                        // ident fenetre secondaire
    ldr x1,qAdrstIMGPNG1                 // structure fichier image PNG
    mov x2,50
    mov x3,200
    bl affichageImagePNG
    /* affichage meme image  autre fenêtre*/
    mov x0,x21                           // ident fenetre principale
    ldr x1,qAdrstIMGPNG1                 // structure fichier image PNG
    mov x2,250
    mov x3,350
    bl affichageImagePNG
...
/* x1 contient la structure fichier image PNG */
creationFenetreSec1:                     // fonction
    ...
    mov x26,x1                         // save de la structure
    ...
    ldr x4,[x26,imagePNG_largeur]
    ldr x5,[x26,imagePNG_hauteur]
    mov x6,#1                          // bordure
...
99:                                   // erreur X11
    ...
    b 100f
100:
    ldp x25,x26,[sp],16               // restaur registre
...
2:
    ...
    rev16 w1,w1
    ldrh w2,[x0,22]              // hauteur image
    rev16 w2,w2
    mul x0,x1,x2
    lsl x0,x0,2                  // * 4 octets par pixel
    affregtit calculplace 0
    bl allocPlace                //allouer la place sur le tas suivant taille
    cbz x0,100f
    str x0,[x26,imagePNG_debut_pixel] // save adresse tas dans structure
    //affregtit finlect 0
fermetureFic:
    mov x0,x27                  // Fd  fichier
...
png_read_from_memPNG:                     // fonction
    ...
    ldr x1,[x0,imagePNG_datas]         // init datas
    ldr x3,[x0,imagePNG_offset]        // init compteur
    mov x2,0
1:                                 // boucle de copie 
    ldrb w4,[x1,x3]
    strb w4,[x21,x2]
    ...
    cmp x2,x22
    blt 1b
    str x3,[x0,imagePNG_offset]       // maj offset
    mov x0,0
100:
...
1:
    ...
    add x2,x27,imagePNG_largeur      // adresse reception largeur
    add x3,x27,imagePNG_hauteur      // adresse reception hauteur
    ...
    ldr x0,[x27,imagePNG_hauteur]  // hauteur image
    ...
    mov x0,x22
    ldr x1,[x27,imagePNG_debut_pixel] // recup addresse zone pixel sur le tas
    mov x2,0                  // indice table pointeurs
    ldr x3,[x27,imagePNG_largeur]  // largeur image
    ldr x4,[x27,imagePNG_hauteur]  // hauteur image
...
3:
    ...
    ldr x0,[x27,imagePNG_debut_pixel] // table des pixels
    ldr x1,[x27,imagePNG_largeur]  // largeur image
    ldr x2,[x27,imagePNG_hauteur]  // hauteur image
...
creationImageX11PNG:                     // fonction
    ...
    ldr x5,[x24,#imagePNG_debut_pixel]      // adresse image PNG
    ldr x6,[x24,#imagePNG_largeur]
    ldr x7,[x24,#imagePNG_hauteur]
    ...
    str x0,[x24,#imagePNG_imageX11]         // maj adresse image X11 dans structure
    b 100f
99:
...
affichageImagePNG:                     // fonction
    ...
    mov x6,x2                     // position X de l'image dans la destination
    mov x7,x3                     // position Y de l'image dans la destination
    mov x0,x19                   // display
    ldr x2,qAdrptGC              // contexte graphique 
    ldr x2,[x2]
    ldr x3,[x24,imagePNG_imageX11]   // adresse de l'image
    mov x4,0                     // position X de l'image dans la source
    mov x5,0                     // position Y de l'image dans la source
    ldr x9,[x24,imagePNG_largeur]
    ldr x10,[x24,imagePNG_hauteur]
    stp x9,x10,[sp,-16]!
    bl XPutImage
    add sp,sp,16                // alignement pile
    cmp x0,0
...
/***************************************************/
/* x0 Nom du fichier image png */
/* x1 Nom de la structure fichier image */
/* x2 identifiant de la fenêtre  */
/* x3 position X */
/* x4 position Y */
chargementImage1:                     // fonction
    stp x24,lr,[sp,-16]!         // save  registres 
    stp x25,x26,[sp,-16]!         // save  registres 
    stp x27,x28,[sp,-16]!         // save  registres 
    mov x24,x1                   // save structure 
    mov x25,x2                   // identifiant fenêtre 
    mov x26,x3
    mov x27,x4
    /* chargement d'une image de type PNG */
    /* x0 = nom du fichier et x1 nom de la structure */
    bl lectfichierPNG                   // lecture fichier image
    cbz x0,99f
    mov x0,x24                 // structure fichier image PNG
    bl chargePNGMem                     // conversion image PNG
    cbz x0,99f
    /* création de l'image X11   */
    mov x0,x24                // pointeur structure image
    bl creationImageX11PNG
    cbz x0,99f
    /* affichage de l'image */
    mov x0,x25                // pointeur fenêtre
    mov x1,x24                // pointeur structure image
    mov x2,x26                // position X
    mov x3,x27                // position Y
    bl affichageImagePNG
    cbz x0,99f
...
100:
    ...
    ldp x24,lr,[sp],16           // restaur des  2 registres
    ret                          // retour adresse lr x30
qAdrszNomImage1:            .quad szNomImage1
qAdrstIMGPNG1:              .quad stIMGPNG1
/*********************************************************************/
